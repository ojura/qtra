...
bool install(CubeWidget* cube, const audio_analysis::Analyzer::Options& options, QString& error)
{
    ...
    // No flags, so the host answers to whatever its build recorded about this
    // target. On a build that did not record it, this is refused, which is the
    // behaviour worth showing. RUNTIME_AGENT_PATCH_ACCEPT_INCOMPLETE exists for
    // a module that has decided it can live with missing some callers.
